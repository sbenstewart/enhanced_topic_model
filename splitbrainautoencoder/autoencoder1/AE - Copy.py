...
class AE(object):
    ...
    def cosineSimilarity(self, input):
        ...
        for vector in range(self.conceptSpace.shape[1]):
            ...
            sim = np.dot(self.conceptSpace[:,0], self.conceptSpace[:, vector]) / (np.linalg.norm(self.conceptSpace[:,0]) *
                np.linalg.norm(self.conceptSpace[:, vector]))
...
if __name__ == "__main__":
    ...
    for (filePath, dirNames, fileNames) in os.walk("E:/BigData/Project/AI_PragatheeswaranS_AjayGunaserkaran/Dataset_in_text_format/Test"):
        for file in sorted(fileNames):
            ...
            sentenceIndex = sorted(range(len(result)), key=lambda i: result[i])[-6:]
            # print('\nSummary finding')
